...
for(i in 1:3){
  set.seed(chains[i])
  correctonly.1[[i]] <- MCMCglmm(Choose.only.correct.dish ~ Treatment*Trial+Batch, random = ~us(1+Trial):LizardID, family = "categorical", nitt = 5010000, thin = 5000, prior=prior.test, burnin = 10000, data=assocdat, verbose= T)
}
...
correctonly.1.inrm <- list()
  for(i in 1:3){
    set.seed(chains[i])
    correctonly.1.inrm[[i]] <- MCMCglmm(Choose.only.correct.dish ~ Treatment+Trial+Batch, random = ~us(1+Trial):LizardID, family = "categorical", nitt = 5010000, thin = 5000, prior=prior.test, burnin = 10000, data=assocdat, verbose= T)
}
...
for(i in 1:3){
  set.seed(chains[i])
  correctonly.1[[i]] <- MCMCglmm(Choose.only.correct.dish ~ Treatment*Trial+Treatment*Batch, random = ~us(1+Trial):LizardID, family = "categorical", nitt = 5010000, thin = 5000, prior=prior.test, burnin = 10000, data=assocdat, verbose= T)
}
